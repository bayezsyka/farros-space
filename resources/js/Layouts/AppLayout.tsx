...
export default function AppLayout({ children, title }: Props) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-background selection:bg-primary selection:text-primary-foreground">
            ...
            <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                <Container className="flex h-14 md:h-16 items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center space-x-6">
                        ...
                        <Link href={route('marketplace.index')} className="text-sm font-medium transition-colors hover:text-primary">
                            Marketplace
                        </Link>
            ...
            {/* Mobile Menu Panel */}
            <div
                className={`fixed top-[57px] right-0 z-40 w-full max-w-[300px] h-[calc(100dvh-57px)] bg-background border-l shadow-xl transition-transform duration-300 ease-in-out md:hidden ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
            >
